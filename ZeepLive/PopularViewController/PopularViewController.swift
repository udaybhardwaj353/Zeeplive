...
extension PopularViewController: UICollectionViewDelegate, UICollectionViewDataSource {
            ...
            if let user = usersList.data?[indexPath.row] {
                ...
                if (user.broadstatus == 1) {
                    ...
                    cell.lblUserStatus.isHidden = false
                       ...
                       animationView.animation = LottieAnimation.named("PkkkK")
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if (usersList.data?[indexPath.item].userStatus == "0")  {
            ...
            nextViewController.apiType = "popular"
            self.navigationController?.pushViewController(nextViewController, animated: true)
...
extension PopularViewController:UICollectionViewDelegateFlowLayout, delegateHomeScreenUsersListCollectionViewCell {
...
extension PopularViewController: ZegoEventHandler {
    
    func createEngine() {
        
        NSLog(" 🚀 Create ZegoExpressEngine Popular Controller.")
...
extension PopularViewController {
    ...
    func getUsersPopularList() {
        
        let url = AllUrls.baseUrl + "livebroadcastList?type=popular&page=\(pageNo)"
