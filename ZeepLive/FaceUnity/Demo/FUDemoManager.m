...
- (void)addDemoViewToView:(UIView *)view originY:(CGFloat)originY {
    ...
    [self segmentBar:self.bottomBar didSelectItemAtIndex:self.integerValue]; // Example, passing index 3
...
- (void)removeDemoView {
    ...
    [self.trackTipLabel removeFromSuperview];
    [self.renderSwitch removeFromSuperview];
}

- (void)showBottomBar {
    [self segmentBar:self.bottomBar didSelectItemAtIndex:self.integerValue];
    [self segmentBar:self.bottomBar shouldSelectItemAtIndex:self.integerValue];
...
- (void)hideBottomBar {
    self.bottomBar.hidden = YES;
    // Add any other views you want to hide
   
    // Hide the currently showing function view if any
      if (self.showingView) {
          [self hideFunctionView:self.showingView animated:YES];
          self.showingView = nil; // Clear the reference to the currently showing view
      }
...
- (void)checkResultWithCompletion:(void (^)(BOOL hidden, NSString * _Nonnull text))completion {
    dispatch_async(dispatch_get_main_queue(), ^{
        BOOL isHidden;
        NSString *message;

        if (self.showingView == self.bodyView) {
            isHidden = [FUAIKit aiHumanProcessorNums] > 0;
            message = FULocalizedString(@"No human body detected");
        } else {
            isHidden = [FUAIKit aiFaceProcessorNums] > 0;
         //   message = FULocalizedString(@"No human face detected");
        }
        
        self.trackTipLabel.hidden = isHidden;
        self.trackTipLabel.text = message;

        if (completion) {
            completion(isHidden, message);
        }
    });
}
...
- (void)checkAITrackedResult {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.showingView == self.bodyView) {
            ...
            self.trackTipLabel.text = FULocalizedString(@"No human body detected");
        } else {
            self.trackTipLabel.hidden = [FUAIKit aiFaceProcessorNums] > 0;
            self.trackTipLabel.text = FULocalizedString(@"No human face detected");
        }
    });
}
